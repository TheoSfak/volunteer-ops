...
// Home-team captain registry, keyed by user id — client-side twin of
// teamCaptains() in includes/functions-warroom.php, same shipped-once-then-
// polled-refresh shape as k9Handlers above. Deliberately unrelated to
// team.leader_id (the existing per-mission Ομάδες Αποστολής leader star).
let teamCaptains = <?= json_encode(teamCaptains(), JSON_UNESCAPED_UNICODE) ?>;
...
function renderRestrictedAreaBreachesList(items) {
    ...
    list.innerHTML = items.map(b => {
        ...
        <div class="border ${resolved ? 'border-secondary' : 'border-danger'} rounded p-2 mb-2${resolved ? ' opacity-75' : ''}">
            <div><strong>${resolved ? '✅' : '⚠️'} ${b.team_label}</strong> — ${guestNameHtml(b.user_name, b.is_external, b.home_team_name, b.home_team_color_bg, b.home_team_color_fg, b.guest_country_code)}${k9BadgeHtml(b.user_id, true)}${captainBadgeHtml(b.user_id, true)}</div>
