...
if (isPost()) {
    ...
    if (post('action') === 'record_payment') {
        ...
        if (!$volunteer || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $paymentDate) || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $requestedExpiryDate)) {
            ...
            dbInsert("INSERT INTO volunteer_subscriptions
                (user_id, payment_date, expiry_date, renewal_kind, coverage_years, amount, payment_method, receipt_original_name, receipt_stored_name, notes, created_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
    ...
    if (post('action') === 'edit_subscription') {
        ...
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $paymentDate) || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $expiryDate)) {
            ...
            $old = dbFetchOne("SELECT * FROM volunteer_subscriptions WHERE id = ?", [$subscriptionId]);
            if ($old) {
                $amount = str_replace(',', '.', trim(post('amount')));
                $reset = $expiryDate !== $old['expiry_date'];
                dbExecute("UPDATE volunteer_subscriptions SET payment_date = ?, expiry_date = ?, amount = ?, payment_method = ?, notes = ?, renewal_kind = ?, reminder_sent_3m = ?, reminder_sent_1m = ?, reminder_sent_1w = ?, reminder_sent_expired = ? WHERE id = ?", [$paymentDate, $expiryDate, $amount === '' ? null : $amount, trim(post('payment_method')) ?: null, trim(post('notes')) ?: null, post('renewal_kind') === 'REACTIVATION' ? 'REACTIVATION' : 'RENEWAL', $reset ? 0 : $old['reminder_sent_3m'], $reset ? 0 : $old['reminder_sent_1m'], $reset ? 0 : $old['reminder_sent_1w'], $reset ? 0 : $old['reminder_sent_expired'], $subscriptionId]);
...
if (get('export') === 'excel') {
    ...
    fputcsv($out, ['Εθελοντής', 'Email', 'Ημερομηνία Πληρωμής', 'Ημερομηνία Λήξης', 'Έτη κάλυψης', 'Τύπος', 'Ποσό', 'Τρόπος Πληρωμής', 'Κατάσταση', 'Σημειώσεις'], ';');
    foreach ($rows as $row) {
        ...
        fputcsv($out, [
            ...
            $row['amount'] ?? '', $row['payment_method'] ?? '', $status, $row['notes'] ?? ''
        ], ';');
    }
    fclose($out);
    exit;
...
<thead><tr><th>Εθελοντής</th><th>Πληρωμή</th><th>Λήξη</th><th>Έτη</th><th>Ποσό</th><th>Τρόπος</th><th>Απόδειξη</th><th>Κατάσταση</th><th></th></tr></thead><tbody>
...
<tr><td><a href="volunteer-view.php?id=<?= $row['user_id'] ?>"><?= h($row['volunteer_name']) ?></a></td><td><?= formatDate($row['payment_date']) ?></td><td><?= formatDate($row['expiry_date']) ?></td><td><?= (int)($row['coverage_years'] ?? 1) ?></td><td><?= $row['amount'] !== null ? number_format((float)$row['amount'], 2, ',', '.') . ' €' : '—' ?></td><td><?= h($row['payment_method'] ?: '—') ?></td><td><?php if ($row['receipt_stored_name']): ?><a class="btn btn-sm btn-outline-secondary" href="subscription-receipt.php?id=<?= $row['id'] ?>"><i class="bi bi-file-earmark-text"></i> <?= h($row['receipt_original_name']) ?></a><?php else: ?>—<?php endif; ?></td><td><span class="badge bg-<?= $badge ?>"><?= $label ?></span></td><td><a class="btn btn-sm btn-outline-primary" href="subscriptions.php?filter=<?= h($filter) ?>&edit=<?= $row['id'] ?>"><i class="bi bi-pencil"></i></a></td></tr>
<?php endforeach; ?>
<?php if (!$rows): ?><tr><td colspan="9" class="text-center text-muted py-4">Δεν υπάρχουν καταχωρημένες συνδρομές.</td></tr><?php endif; ?>
</tbody></table></div></div>
