...
$operationalDefaultTypeIds = array_map('intval', array_column(
...
$selectedMissionTypeId = (int)($mission['mission_type_id'] ?? post('mission_type_id', 1));
$showInOpsChecked = isPost()
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="form-check mb-3 border rounded p-3 bg-light">
                        <input class="form-check-input" type="checkbox" id="show_in_ops" name="show_in_ops"
                               <?= $showInOpsChecked ? 'checked' : '' ?>>
                        <label class="form-check-label fw-semibold" for="show_in_ops">
                            <i class="bi bi-broadcast-pin text-danger me-1"></i>Εμφάνιση στο Επιχειρησιακό
                        </label>
...
(() => {
    ...
    const showInOps = document.getElementById('show_in_ops');
    const isNewMission = <?= $isEdit ? 'false' : 'true' ?>;
    const defaultOperationalTypes = <?= json_encode($operationalDefaultTypeIds) ?>.map(String);
    if (typeSelect && showInOps && isNewMission) {
        typeSelect.addEventListener('change', () => {
            showInOps.checked = defaultOperationalTypes.includes(typeSelect.value);
        });
    }
})();
</script>
