...
<div class="card pp-card accent-<?= $subscriptionColor ?> mb-4">
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2"><h5 class="mb-0"><i class="bi bi-cash-coin text-<?= $subscriptionColor ?> me-2"></i>Η Ετήσια Συνδρομή μου</h5><div class="d-flex gap-2"><?php if ($canIrisRenew): ?><button type="button" class="btn btn-primary fw-bold" data-bs-toggle="modal" data-bs-target="#irisRenewalModal"><i class="bi bi-arrow-repeat me-1"></i>Ανανέωση συνδρομής</button><form method="post" class="d-inline"><?= csrfField() ?><input type="hidden" name="action" value="report_iris_payment"><button class="btn btn-success fw-bold" <?= !$myIrisRequest || $myIrisRequest['status'] !== 'PREPARED' ? 'disabled title="Επιλέξτε πρώτα τη διάρκεια ανανέωσης"' : '' ?>><i class="bi bi-check2-circle me-1"></i>Ενημέρωση πληρωμής</button></form><?php else: ?><span class="d-inline-block iris-renewal-disabled" tabindex="0" data-tooltip="Η ανανέωση ενεργοποιείται 3 μήνες πριν από τη λήξη της συνδρομής."><button type="button" class="btn btn-outline-secondary fw-bold" disabled><i class="bi bi-arrow-repeat me-1"></i>Ανανέωση συνδρομής</button></span><?php endif; ?></div></div>
    <div class="card-body">
