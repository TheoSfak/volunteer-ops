...
CREATE TABLE IF NOT EXISTS `users` (
    ...
    `is_team_captain` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Captain of this user''s home team (volunteer_teams/mission_visitor_tags) — drives the ⭐ captain badge next to this user''s name app-wide. Independent of mission_teams.leader_id, which is a per-mission Action Room squad leader, not a home-team role.',
...
CREATE TABLE IF NOT EXISTS `missions` (
    ...
    `is_locked` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'When set, only missions_manage admins can add volunteers (manual_add_volunteer) — self-serve apply is blocked on mission-view.php and shift-view.php',
...
-- =============================================
-- MISSION TEAMS (War Room team assignments)
-- =============================================
...
CREATE TABLE IF NOT EXISTS `mission_photos` (
    ...
    `accuracy_m` DECIMAL(8,2) NULL COMMENT 'Device-reported uncertainty radius of the fix, metres; NULL = not reported',
...
-- =============================================
-- MISSION ORDERS (War Room location/photo/video request tracking)
-- =============================================
CREATE TABLE IF NOT EXISTS `mission_orders` (
    `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `mission_id` INT UNSIGNED NOT NULL,
    `order_type` ENUM('location','photo','video','task','message','return_to_base','route','charge_phone','live','speak') NOT NULL,
    `task_text` TEXT NULL,
    `created_by` INT UNSIGNED NOT NULL,
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (`mission_id`) REFERENCES `missions`(`id`) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS `mission_order_recipients` (
    `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
-- system. Every other row in that feed is generated by the app reacting to
-- something; this is the only source a human writes directly.
-- author_id is NULLABLE with ON DELETE SET NULL, deliberately unlike
-- mission_shortage_reports.reporter_id: a log entry must survive its author
-- leaving the organisation rather than deleting itself years later.)
-- =============================================
CREATE TABLE IF NOT EXISTS `mission_activity_notes` (
    ...
    `note` TEXT NOT NULL,
    `staff_only` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Keeps a sensitive entry off the volunteers copy of the same feed',
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (`mission_id`) REFERENCES `missions`(`id`) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS `mission_missing_persons` (
    ...
    `disappearance_circumstances` TEXT NULL,
    `likely_direction` VARCHAR(255) NULL,
    `witness_accounts` TEXT NULL,
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (`mission_id`) REFERENCES `missions`(`id`) ON DELETE CASCADE,
...
-- =============================================
-- MISSION SOS ALERTS (War Room GPS-tagged emergency alarm lifecycle)
-- =============================================
CREATE TABLE IF NOT EXISTS `mission_sos_alerts` (
    `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `mission_id` INT UNSIGNED NOT NULL,
    `user_id` INT UNSIGNED NOT NULL,
...
-- =============================================
-- MASS-CASUALTY TRIAGE («Μαζικό Συμβάν», v166). mission_mci is the per-mission
-- switch plus the casualty collection point and green area. A casualty is one
-- mission_triage_victims row, identified by the phone's uuid, then by the
-- physical triage card number (unique per mission: the same card is the same
-- person), else by a phone-made T<user>-<n> code. Every START/JumpSTART
-- answer set is a mission_triage_assessments row; the victim's category is the
-- latest by FIELD time. See includes/functions-triage.php.
-- =============================================
CREATE TABLE IF NOT EXISTS `mission_mci` (
...
CREATE TABLE IF NOT EXISTS `mission_triage_victims` (
    `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `mission_id` INT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `mission_triage_bulk` (
    `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `mission_id` INT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `mission_triage_status_log` (
    `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `victim_id` INT UNSIGNED NOT NULL,
    `mission_id` INT UNSIGNED NOT NULL,
