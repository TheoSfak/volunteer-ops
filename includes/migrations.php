  ...
  <tr><td align="center" style="padding:30px 12px;">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:100%;max-width:600px;border-collapse:collapse;background:#ffffff;">
      ...
      <tr><td align="center" style="background:#1a1a2e;padding:28px 32px;text-align:center;">
                ...
                if (!$ns) {
                    dbInsert(
                        ...
                        [
                            ...
                            'Έκδοση Βεβαίωσης Αποστολής',
                            'Μια βεβαίωση συμμετοχής σε αποστολή εκδόθηκε στο όνομά σας (μόνο push/εντός εφαρμογής, όχι email)',
                        ]
                    );
                }
            },
        ...
        [
            'version'     => 131,
            'description' => 'Add missions.fires_overlay_enabled + fire_hotspot_cache table for the NASA FIRMS wildfire-hotspot map overlay. Unlike the weather compass (a global Settings toggle), this one is a per-mission flag flipped live by admins from inside the Action Room itself (mission-fires.php), synced to every viewer through the existing ajax=1 poll — same reasoning as v130\'s weather_cache, kept here rather than only in sql/migrations/ so it actually auto-applies on a plain git-deploy. 2026-08-11: rewrote the column-add to use the standard INFORMATION_SCHEMA pre-check — the original "MariaDB-native IF NOT EXISTS" version was root-causing this migration\'s own previously-unexplained production failures: that ALTER syntax is a hard SQL syntax error on real MySQL 8 (confirmed against a mysql:8.0.46 container), and since the runner stops at the first failure, it was silently blocking every later migration too on any MySQL-backed install, not just this one\'s own two changes.',
            'up' => function () {
                // Standard existence-check-then-plain-ALTER idiom, matching
                // every other migration in this file (see v129/v133) — the
                // MariaDB-only "ADD COLUMN IF NOT EXISTS" form this used to be
                // is a syntax error on real MySQL, which is almost certainly
                // what the "kept silently failing on production" symptom
                // below was actually caused by.
                $columnExists = dbFetchOne(
                    "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
        ...
        [
            'version'     => 135,
            'description' => 'Add ring_index TINYINT UNSIGNED NULL to mission_dispatch_points, mission_routes, and mission_search_areas — tags a row as generated by one of the LPB search-ring shortcuts (war-room.php openDispatchForRing/openRouteForRing/openInteriorSweepForRing/openDivideRingIntoSectors/openAutoAssignForRing) with which ring produced it (0=25%,1=50%,2=75%,3=95%, matching LPB_RING_TABLE\'s own index), NULL for hand-drawn. Backs the "reset ring assignments" button on the missing-person card, which bulk-clears only ring-generated content. mission_search_sectors gets no column of its own — its area_id FK is ON DELETE CASCADE, so deleting a ring-origin area already removes its sectors.',
            'up' => function () {
                $col = dbFetchOne(
                    "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'mission_dispatch_points' AND COLUMN_NAME = 'ring_index'"
                );
                if (!$col) {
                    dbExecute("ALTER TABLE mission_dispatch_points ADD COLUMN ring_index TINYINT UNSIGNED NULL AFTER team_id");
                }
                $col = dbFetchOne(
                    "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'mission_routes' AND COLUMN_NAME = 'ring_index'"
                );
                if (!$col) {
                    dbExecute("ALTER TABLE mission_routes ADD COLUMN ring_index TINYINT UNSIGNED NULL AFTER team_id");
                }
                $col = dbFetchOne(
                    "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'mission_search_areas' AND COLUMN_NAME = 'ring_index'"
                );
                if (!$col) {
                    dbExecute("ALTER TABLE mission_search_areas ADD COLUMN ring_index TINYINT UNSIGNED NULL AFTER label");
                }
            },
        ],
        ...
        [
            'version'     => 139,
            'description' => 'Add mission_activity_notes — hand-typed command-staff entries in the Action Room activity log ("call from EKAB 04:12", a decision taken, anything that happened off-system). Every other row in that feed is generated by the app reacting to something; this is the only source a human writes directly, which is what makes the log a real operations journal rather than only a machine trace. staff_only keeps a sensitive entry off the volunteers\' copy of the same feed. author_id is NULLABLE with ON DELETE SET NULL, deliberately unlike mission_shortage_reports.reporter_id (CASCADE): a log entry must survive its author leaving the organisation, or the mission record silently rewrites itself years later.',
            'up' => function () {
                dbExecute(
                    "CREATE TABLE IF NOT EXISTS `mission_activity_notes` (
                        `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                        `mission_id` INT UNSIGNED NOT NULL,
                        ...
                        INDEX `idx_activity_note_mission` (`mission_id`, `created_at`)
                    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
                );
            },
        ],
        ...
        [
            'version'     => 143,
            'description' => 'Restyle member_application_confirmation to match every other email template (the outer/header/body/footer shape from migration v10 onward) — v142 shipped it in an older, one-off style. Adds the logo to the header via {{logo_html}} and the org\'s contact details to the footer via the new {{org_contact_line}} (auto-injected by sendNotificationEmail() for every template now, same mechanism as {{logo_html}}).',
            'up' => function () {
                $body = '<div style="background:#eef2f7;padding:28px 0 40px;font-family:Helvetica Neue,Arial,sans-serif;">'
                    . '<div style="max-width:600px;margin:0 auto;">'
                    . '<div style="background:#005596;padding:30px 40px 26px;border-radius:12px 12px 0 0;text-align:center;">'
                    . '{{logo_html}}'
                    . '<p style="color:rgba(255,255,255,0.7);font-size:11px;letter-spacing:2px;text-transform:uppercase;margin:0 0 8px;">{{app_name}}</p>'
                    . '<div style="font-size:36px;line-height:1;margin:0 0 8px;">&#9989;</div>'
                    . '<h1 style="color:#fff;margin:0;font-size:23px;font-weight:700;line-height:1.3;">Λάβαμε την Αίτησή σας</h1>'
                    . '</div>'
                    . '<div style="background:#fff;padding:36px 40px 40px;border-radius:0 0 12px 12px;box-shadow:0 4px 20px rgba(0,0,0,0.07);">'
                    . '<h2 style="color:#1f2937;font-size:18px;font-weight:700;margin:0 0 14px;">Γεια σας {{applicant_name}},</h2>'
                    . '<p style="color:#4b5563;line-height:1.65;font-size:15px;margin:0 0 14px;">Σας ευχαριστούμε για το ενδιαφέρον σας να γίνετε μέλος της ομάδας μας. Λάβαμε την αίτησή σας για ένταξη ως νέο μέλος και θα την εξετάσουμε το συντομότερο δυνατό.</p>'
                    . '<div style="background:#eff6ff;border-left:4px solid #005596;padding:14px 20px;border-radius:0 8px 8px 0;margin:20px 0;">'
                    . '<p style="color:#005596;font-weight:600;font-size:14px;margin:0;">&#128172; Κάποιο μέλος της ομάδας μας θα επικοινωνήσει μαζί σας σύντομα.</p>'
                    . '</div>'
                    . '</div>'
                    . '<div style="text-align:center;padding:18px 0 0;color:#9ca3af;font-size:12px;line-height:1.9;">'
                    . '<p style="margin:0;"><strong style="color:#6b7280;">{{app_name}}</strong> &bull; Σύστημα Διαχείρισης Εθελοντών</p>'
                    . '{{org_contact_line}}'
                    . '<p style="margin:8px 0 0;">Αυτό το μήνυμα στάλθηκε αυτόματα από το σύστημα.</p>'
                    . '</div>'
                    . '</div>'
                    . '</div>';

                dbExecute(
                    "UPDATE email_templates SET body_html = ?, available_variables = ?, updated_at = NOW() WHERE code = 'member_application_confirmation'",
                    [$body, '{{app_name}}, {{applicant_name}}, {{logo_html}}, {{org_contact_line}}']
                );
            },
        ],
        ...
        [
            'version'     => 164,
            'description' => "«Να με θυμάσαι» on the login page. One row per remembered device: the cookie carries selector:validator, the row keeps only a SHA-256 of the validator, so a leaked table cannot be replayed as a cookie. A table rather than the unused users.remember_token column, because a volunteer's phone and the station PC must each be remembered and each be forgotten on its own logout.",
            'up' => function () {
                dbExecute("CREATE TABLE IF NOT EXISTS user_remember_tokens (
                    id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                    user_id INT UNSIGNED NOT NULL,
        ...
        [
            'version'     => 165,
            'description' => "Dispatch points/areas get the depart → arrive → complete cycle sectors and routes already have («Ξεκινάω», «Έφτασα», «Ολοκληρώθηκε»). One row per dispatch per TEAM — scope_key 't<team_id>', or 'u<user_id>' for a volunteer with no team — because the first member to press moves the whole team on. «Ελήφθη» stays per person in mission_dispatch_receipts, and arrival is still also written to mission_dispatch_acks for whoever pressed it, which the activity log, the report and the assistant read.",
            'up' => function () {
                dbExecute("CREATE TABLE IF NOT EXISTS mission_dispatch_progress (
                    id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                    dispatch_id INT UNSIGNED NOT NULL,
