...
<head>
    ...
    <!-- Flatpickr for date/time pickers (version pinned so the SRI hash below stays valid — the
         old unversioned URL floated to whatever jsdelivr resolved "latest" to at request time) -->
    ...
    <script src="https://cdn.jsdelivr.net/npm/sortablejs@1.15.1/Sortable.min.js" integrity="sha384-qtc/K/67UaErkw0RSqYJTesfov/6RB6jr33tjDquXkK4SxbZO/ep3+NVgZzhf4Wz" crossorigin="anonymous"></script>
    ...
    <style>
        ...
        * {
        ...
        /* Home-team captain badge (users.is_team_captain) — same pill shape
           ...
           unstyled ⭐ used elsewhere for mission_teams.leader_id (a completely
           different, per-mission Action Room concept — see captainBadgeHtml()
           in includes/functions-warroom.php). */
        .captain-badge {
            display: inline-flex;
            align-items: center;
            gap: 3px;
...
<?php unset($__pr,$__prName,$__prColor); endif; ?>
    ...
    <nav class="sidebar" id="sidebar">
        ...
        <ul class="nav flex-column">
            ...
            <?php endif; ?>
            <?php if (isSystemAdmin() || hasPagePermission('skills_manage')): ?>
            <li class="nav-item">
            ...
            <?php endif; ?>
            <?php if (isSystemAdmin() || hasPagePermission('certificates_manage')): ?>
            <li class="nav-item">
            ...
            <?php if (isSystemAdmin() || hasPagePermission('inventory_manage') || hasPagePermission('inventory_view')): ?>
            <li class="nav-item">
                <a class="nav-link <?= $currentPage === 'inventory-shelf' ? 'active' : '' ?>" href="inventory-shelf.php">
                    <i class="bi bi-grid-3x3"></i> Υλικά Ραφιού
                </a>
            </li>
            <?php if (isSystemAdmin() || hasPagePermission('inventory_manage')): ?>
            <li class="nav-item">
            ...
            <div class="d-flex align-items-center flex-shrink-0">
                ...
                <div class="dropdown">
                    <button class="btn btn-link text-dark dropdown-toggle text-decoration-none" type="button" id="userDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                        ...
                        <?php endif; ?>
                        <span class="user-name-text d-none d-sm-inline"><?= $currentUser ? guestNameHtml($currentUser['name'], (bool)($currentUser['is_external'] ?? false), $currentUser['home_team_name'] ?? null, $currentUser['home_team_color'] ?? null, $currentUser['guest_country_code'] ?? null) . k9BadgeHtml((int)($currentUser['id'] ?? 0), true) . captainBadgeHtml((int)($currentUser['id'] ?? 0), true) : h('Χρήστης') ?></span>
